...
class RealEstateDataProvider:
    ...
    def get_monthly_supply_stats(
        ...
        raw_df["month"] = pd.to_datetime(

            raw_df["date_code"], errors="coerce"
    ...
    def get_annual_supply_ratio_stats(
        ...
        supply_counts = raw_df[raw_df["supply_sets"] == 1].groupby(

            "year", observed=False

        ).size()

        trade_counts = raw_df[raw_df["trade_sets"] == 1].groupby(
        ...
        ratio_series = (
            ...
            supply_counts.div(trade_base).mul(100).fillna(0).round(1).rename(col_names[0])
    ...
    def _get_yearly_resale_trade_data(
        ...
        *,
        ...
        raw_df = self._fetch_raw_data_or_raise(columns=columns, function_key=function_key)
    ...
    def get_resale_avg_price_stats(
        ...
        result["avg_unit_price"] = result["avg_unit_price"].fillna(0).round(0).astype(int)
