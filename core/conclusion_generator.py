...
class ConclusionGenerator:
    ...
    def _build_share_conclusion(
        ...
        *,
        ...
        numeric_cols = [
            ...
            col for col in df.select_dtypes(include="number").columns if col != bucket_col

        ]

        if not numeric_cols:

            raise ValueError(f"No numeric metric column found for share analysis: {bucket_col}")
        ...
        working_df[count_col] = pd.to_numeric(working_df[count_col], errors="coerce").fillna(0)
    ...
    def get_yoy_price_change_conclusion(self, df_data: pd.DataFrame) -> dict[str, str]:
        ...
        trend_label, trajectory_type, secular_direction = self._trend_direction_words(price_delta)
        ...
        return {
            ...
            "Trend_Market_Absorption_State": "seller-favorable" if price_delta >= 0 else "buyer-favorable",
    ...
    def get_supply_ratio_conclusion(self, df_data: pd.DataFrame) -> dict[str, str]:
        ...
        direction_label, trajectory_type, secular_direction = self._trend_direction_words(

            terminal_ratio - base_ratio
    ...
    def get_area_year_pivot_conclusion(self, df_data: pd.DataFrame) -> dict[str, str]:
        ...
        if not trade_cols:
            ...
            trade_cols = [
                ...
                col for col in df.select_dtypes(include="number").columns if col != "area_range"
        ...
        share_text = f"{dominant_share:.1f}" if dominant_share < 10 else f"{dominant_share:.0f}"
    ...
    def get_annual_supply_trade_conclusion(self, df_data: pd.DataFrame) -> dict[str, str]:
        ...
        if trade_col is None:
            ...
            candidates = [col for col in df.columns if col not in {year_col, supply_col}]
