...
def build_react_agent_graph(
    *,
    ...
    system_prompt = (
        ...
        "- Do not describe tool usage in natural language when a tool call is possible.\n"
        "- Finish by using the provided `ReactJudgeOutput` tool exactly once for the final verdict.\n"
        "- Do not add extra natural-language explanation after the final verdict.\n"
        "- If a tool call fails, retry with corrected arguments, but keep retries bounded (at most 2 retries per failed step).\n"
        "- If the input slide is factually correct, use the `ReactJudgeOutput` tool with `has_issue=false` and do not edit anything.\n"
        "- If the input slide has a factual mismatch, you MUST correct the PPT before finishing.\n"
        "- When `has_issue=true`, you MUST complete a real textbox edit by calling `apply_textbox_edit` before using the `ReactJudgeOutput` tool.\n"
        "- `apply_textbox_edit` is the execution step that performs the correction. Use it only after you are already confident the slide has a factual issue and you know the corrected text.\n"
        ...
        "- Otherwise the verdict is `has_issue=false`."
    )
    return create_agent(
        model=model,
...
def build_react_input_messages(image_path: Path) -> dict[str, Any]:
    user_prompt = (
        ...
        "If the slide is correct, use the final verdict tool with `has_issue=false`.\n"
        "If the slide has a factual issue, you must correct the PPT by calling `apply_textbox_edit` before using the final verdict tool with `has_issue=true`.\n"
        "Use the provided tools directly rather than describing tool usage in natural language.\n"
        "Finish with the final verdict tool only once."
    )
    return {
        "messages": [
            HumanMessage(
